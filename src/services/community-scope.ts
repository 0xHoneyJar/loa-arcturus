 ...
 *   upstream file:   packages/services/community-scope.ts
 ...
 *   classification:  TRANSITIVE dep (NOT in spec §2 list) — required to import a §2 file; see PROVENANCE.md
 *
 ...
 * noted here:
 *   - 2026-06-11, Sprint 4 (Task 4.2/4.3, FR-6): replaced the two
 *     `SET LOCAL app.community_id = $1` statements (withCommunityScope,
 *     withCommunityBoundary) with `SELECT set_config('app.community_id', $1,
 *     true)`. PostgreSQL does NOT accept bind parameters in `SET LOCAL` (it
 *     raises `syntax error at or near "$1"`); `set_config(name, value, is_local=
 *     true)` is the canonical, parameterizable, transaction-local equivalent.
 *     This surfaced the FIRST time the real JS settle() path ran against a live
 *     Postgres in the acceptance harness — Sprints 1-3 exercised the insert
 *     SHAPES via psql / string-interpolated SET LOCAL, never this parameterized
 *     query. Behaviour is otherwise identical (same GUC, same transaction scope,
 *     same rollback semantics). No other logic changed.
 * The original loa-freeside header and body otherwise follow.
...
export async function withCommunityScope<T>(
  ...
  try {
    ...
    // (Postgres rejects bind params in `SET LOCAL`). Transaction-scoped GUC.
    await client.query('SELECT set_config($1, $2, true)', ['app.community_id', communityId]);
...
export async function withCommunityBoundary<T>(
  ...
  // set_config(..., is_local=true) == SET LOCAL, but parameterizable (Postgres
  // rejects bind params in `SET LOCAL`). Transaction-scoped GUC; caller's txn.
  await client.query('SELECT set_config($1, $2, true)', ['app.community_id', communityId]);
  return fn(client);
}
