 ...
 *   upstream file:   packages/routes/x402.routes.ts
 ...
 * noted here:
 *   - 2026-06-10, Sprint 2 (Task 2.4, FR-4): the facilitator block is now
 *     config-driven. The hardcoded chain:'base' / token:'USDC' / default
 *     chainId=8453 are replaced by a chain resolved from CHAIN_CONFIGS
 *     (default 'bepolia', the PoC chain) plus a configurable settlement token
 *     symbol. No settlement logic changed — only the advertised facilitator
 *     descriptor.
 ...
 *   2. Client pays on-chain (self-broadcast transfer on the configured chain,
 *      default Bepolia/HONEY) with nonce binding
...
import { CHAIN_CONFIGS } from '../core/ports/chain-provider.js';
...
export interface X402RouteDeps {
  ...
  /**
   * Settlement chain, as a key into CHAIN_CONFIGS (FR-4, Sprint 2 Task 2.4).
   * Default 'bepolia' — the Proof-of-Revenue PoC chain (80069). The advertised
   * facilitator chain name + chain_id are derived from CHAIN_CONFIGS[chainKey],
   * replacing the previously hardcoded Base/8453.
   */
  chainKey?: string;
  /**
   * Settlement token SYMBOL advertised in the facilitator descriptor (FR-4).
   * Default 'HONEY' (Bepolia); 'USDC' is also accepted. This is a display
   * symbol only — NOT a token contract address or transfer interface. The PoC
   * uses the self-broadcast model (payer sends a normal transfer, hands the
   * server the tx_hash), so no HONEY token-standard assumption is made here;
   * the HONEY contract/standard remains UNVERIFIED (Task 2.5, OPEN-2).
   */
...
export function createX402Router(deps: X402RouteDeps): Router {
  ...
  const {
    ...
    facilitatorAddress,
    chainKey = 'bepolia',
    settlementToken = 'HONEY',
  } = deps;

  // Resolve the settlement chain from config (FR-4). No hardcoded chain/token/
  // chainId remains on the facilitator path: the advertised descriptor below is
  // built entirely from CHAIN_CONFIGS[chainKey] + settlementToken.
  const chainConfig = CHAIN_CONFIGS[chainKey];
  if (!chainConfig) {
    throw new Error(
      `Unknown x402 settlement chainKey '${chainKey}'. ` +
      `Available: ${Object.keys(CHAIN_CONFIGS).join(', ')}`,
    );
  }
  const facilitatorDescriptor = {
